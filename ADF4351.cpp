...
void ADF4351::write(const uint32_t frequency)
{
   ...
   if (fPll != oldPllFrequency)
   {
        ...
        double nPll = fPll * 80.0 / 1000000.0;
        uint32_t intPll = (int) nPll;
        uint fracPll = (int) (1250 * (nPll - intPll));
        ...
        // write R4
        if (!isPressed("ptt")) // more power from PLL in RX mode
        {
            uint8_t r4[] = {0x00, 0xD0, 0x14, 0x3C};
            writePLL(r4);
        } 
        else
        {
            uint8_t r4[] = {0x00, 0xD0, 0x14, 0x34};
            writePLL(r4);
        } 
  
        // write R3
        uint8_t r3[] = {0x00, 0x80, 0x04, 0xB3};
        writePLL(r3);
    
        // write R2
        uint8_t r2[] = {0x00, 0x06, 0x4E, 0x42};
        writePLL(r2);        
  
        // write R1
        uint8_t r1[] = {0x08, 0x00, 0xA7, 0x11};
        writePLL(r1);
